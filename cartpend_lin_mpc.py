...
P = ca.SX.sym('P', 2*n_states+n_states*(N+1)+n_controls*N)
...
def cartpend_lin(x, u, xp, up):
    ...
    A = ca.jacobian(cartpend(xp, up), xp)
    B = ca.jacobian(cartpend(xp, up), up)
...
f_lin = ca.Function('f_lin', [states, controls, XP, UP], [RHS_lin])

cost_fn = 0  # cost function
x_init = P[:n_states]  # constraints in the equation

xref = P[n_states:2*n_states]

g = X[:,0]-x_init

for k in range(N):
    if k < N-1:
        x_fixed = P[2*n_states+(k+2)*n_states:2*n_states+(k+3)*n_states]
        u_fixed = P[2*n_states+N*n_states+k+1:2*n_states+N*n_states+k+2]
    else:
        x_fixed = P[2*n_states+(k+1)*n_states:2*n_states+(k+2)*n_states]
        u_fixed = P[2*n_states+N*n_states+k:2*n_states+N*n_states+k+1]
    st = X[:,k]
    con = U[:,k]
    st_next = X[:,k+1]
    cost_fn = cost_fn+((st-xref).T@Q@(st-xref))+(con.T@R@con.T)
    
    f_value = f_lin(st, con, x_fixed, u_fixed)
...
if __name__ == '__main__':
    ...
    while (mpc_iter < sim_time/T):
        ...
        X0 = ca.reshape(X0, n_states*(N+1), 1)
        U0 = ca.reshape(U0, n_controls*N, 1)
        args['x0'] = ca.vertcat(X0, U0)
    ...
    sim.simulate(x_sim,tspan,L,M,T,'Simulation_mpc_lin_up.mp4')